...
class Run:
    ...
    def GetE(self, time, merged=False, interp="nearest", all_primal=True, **kwargs):
        ...
        return VectorField(h)
#test
    def GetM(self, time, merged=False, interp ="nearest", all_primal=True, **kwargs):
        if merged:
            all_primal=False
        hier = self._get_hierarchy(time, "ions_momentum_tensor.h5", **kwargs)
        if not all_primal:
            return self._get(hier, time, merged, interp)
        h = compute_hier_from(_compute_to_primal, hier, Mxx="Mxx", Mxy="Mxy", Mxz="Mxz", Myy="Myy", Myz="Myz", Mzz="Mzz")
        return(h)
...
#test
  ...
  #Test      
    def GetTe(self, time, merged=False, interp="nearest", all_primal=True):
        hier = self._get_hierarchy(time, "ions_density.h5")
        print(dir(hier._sim))
        Te = hier.sim.electrons.closure.Te
        if merged:
            all_primal=False
        if not all_primal:
            return self._get(hier, time, merged, interp), Te
        h = compute_hier_from(_compute_to_primal, hier, scalar="rho")
        return ScalarField(h), Te
    #test
